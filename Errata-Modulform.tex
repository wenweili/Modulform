...
\title{\bfseries 《模形式初步》勘误表 \\ 跨度: 2024 年 8 月迄今}
\author{李文威}
\date{\today}
...
\begin{document}
	...
	\begin{Errata}
		\item[定义 A.6.1 的第一个显示公式]
		\Orig $f(c + iy) \ll (1 + |y|)^M$
		\Corr $F(c + iy) \ll (1 + |y|)^M$
		\Thx{感谢王子宣指正}
	
		\item[定义 B.1.1 最后的显示公式]
		\Orig $\rho^W_{V_i}$
		\Corr $\rho^V_{W_i}$
		\Thx{感谢王子宣指正}
		
		\item[\S B.3 开头第二段第一句]
		\Orig $A' \xrightarrow{f'} B$
		\Corr $A' \xrightarrow{f'} B'$
		\Thx{感谢郝亮指正}
	\end{Errata}
\end{document}
